...
import {
  fetchUserStarTerraStakingPools,
  fetchUserLpData,
  getStarTerraPools,
} from "./pools";
...
export const getStarTerraAccount = async (address: string) => {
  try {
    const [userGovData, userLpStaking, poolInfo, userLpInfo] = await fetchData(
      address
    );
    const sttPrice = getPrice(poolInfo);
    const { govData, govRewardsTotal, govStakedTotal } = getStarTerraGov(
      userGovData,
      sttPrice
    );
    const starTerraPools = getStarTerraPools(
      userLpStaking,
      userLpInfo,
      poolInfo,
      sttPrice
