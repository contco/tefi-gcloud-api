...
const fetchPylonData = async (address: string) => {
  const mineOverviewPromise = fetchData(
    PYLON_API_ENDPOINT + "mine/v1/overview"
  );
  const governanceOverviewPromise = fetchData(
    PYLON_API_ENDPOINT + "governance/v1/overview"
  );
  const poolOverViewPromise = fetchData(
    PYLON_API_ENDPOINT + "liquidity/v1/overview"
  );
  const getAccountDetailsPromise = fetchData(
    PYLON_API_ENDPOINT + `account/v1/${address}`
  );
  const mineStakingPromise = fetchData(
    PYLON_API_ENDPOINT + `governance/v1/status/${address}`
  );
  const minePoolPromise = fetchData(
    PYLON_API_ENDPOINT + `liquidity/v1/status/${address}`
  );
  const airdropPromise = fetchData(
    PYLON_API_ENDPOINT + `mine/v1/airdrop/${address}`
  );
  ...
  if (minePoolData?.data) {
    ...
    if (stakedBalance || balance) {
      ...
      const totalLpUstValue = (
        stakeableLpUstValue + stakedLpUstValue
      ).toString();
...
export const getPylonAccount = async (address: string) => {
  try {
    const [
      ...
      airdropData,
      pylonGateway,
    ]: any = await fetchPylonData(address);
    if (mineOverview) {
      const { priceInUst } = mineOverview.data;
      const lpValue = getLpValue(mineOverview?.data?.liquidityInfo, priceInUst);
      const { pylonHoldingsSum, pylonHoldings } = getPylonHoldings(
        priceInUst,
        getAccountDetails
      );
      const gov = getPylonGov(
        priceInUst,
        mineStakingData,
        governanceOverview?.data?.apy
      );
      ...
      const { pylonAirdropSum, pylonAirdrops } = await getPylonAirdrops(
        address,
        priceInUst,
        airdropData?.data
      );
      const { gatewayPoolData, gatewayRewardsSum, gatewayDepositsSum } =
        pylonGateway;
      const pylonTotal = {
        pylonHoldingsSum,
        pylonPoolSum,
