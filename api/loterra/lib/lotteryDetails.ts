...
export const getLotteryDetails = async (
  ...
  if (lottery_id) {
    try {
      ...
      const ticketCountsRequest = wasmStoreRequest(
        contracts.lotteryContract,
        count_ticket
      );
      const userCombinationsRequest = wasmStoreRequest(
        contracts.lotteryContract,
        combinationParams
      );
      const jackPotRequest = wasmStoreRequest(
        contracts.lotteryContract,
        jackpotParams
      );
      ...
      return {
        ticketCounts,
        userCombinations: userCombinations?.combination,
        jackpot: recentJacpot,
      };
    } catch (err) {
