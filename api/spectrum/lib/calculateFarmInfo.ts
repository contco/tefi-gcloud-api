...
export const calculateFarmInfos = (
  poolInfo,
  pairStats,
  pairRewardInfos,
  poolResponses
  ...
  for (const key of Object.keys(poolInfo)) {
    if (pairRewardInfos[key] && pairRewardInfos[key].bond_amount) {
      ...
      const stakedLp = parseFloat(pairRewardInfos[key].bond_amount) / MICRO;
      const stakedSpec =
        parseFloat(pairRewardInfos[key]?.pending_spec_reward) / MICRO;
      const stakedSpecValue = getStakedTokenValue(
        stakedSpec,
        poolResponses[contracts.specToken]
      );
      ...
      if (poolInfo[key].farm !== "Spectrum") {
        tokenRewardsStaked =
          parseFloat(pairRewardInfos[key]?.pending_farm_reward) / MICRO;
        const stakedTokenPoolResponse = getStakedTokenPoolResponse(
          poolResponses,
          poolInfo[key].farm
        );
        tokenRewardsStakedValue = getStakedTokenValue(
          tokenRewardsStaked,
          stakedTokenPoolResponse
        );
      }
      ...
      rewardsTotal =
        rewardsTotal +
        parseFloat(tokenRewardsStakedValue) +
        parseFloat(stakedSpecValue);
      const farmInfo = {
  ...
  return {
    farmInfos,
    farmsTotal: farmsTotal.toString(),
