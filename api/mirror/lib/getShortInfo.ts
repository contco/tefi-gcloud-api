...
import { math, MICRO, getLastSyncedHeight } from "@contco/terra-utilities";
...
import { MANTLE_URL, LUNA_DENOM } from "../../constants";
...
export const getRewards = (token, mirPrice, stakingRewards) => {
  const shortRewards = stakingRewards.reward_infos.filter(
    (asset) => asset.is_short === true
  );
  const assetReward = shortRewards.filter(
    (asset) => asset.asset_token === token
  )[0];
    ...
    const mintInfo = await axios.get(MANTLE_URL + `?mintPositions`, {
      params: {
        ...
        variables: {
          ...
          msg: JSON.stringify({
            positions: { owner_addr: address, limit: 30 },
          }),
        },
      },
    });
...
export const getOraclePrice = async (contract) => {
  try {
    const oraclePrice: any = await axios.get(MANTLE_URL + `?oraclePrice`, {
      params: {
...
const getAUSTInfo = async () => {
  ...
  try {
    const austInfo: any = await axios.get(
      MANTLE_URL + `?anchorMarketEpochState`,
      {
        params: {
          ...
          variables: {
            ...
            msg: JSON.stringify({
              epoch_state: { block_height: lastSyncedHeight },
            }),
          },
        },
      }
    ...
    if (austInfo) {
      return JSON.parse(
        austInfo?.data?.data?.WasmContractsContractAddressStore?.Result
      )?.exchange_rate;
    } else return "0";
  } catch (err) {
    return "0";
    ...
    if (contractAddr.includes("terra")) {
      if (contractAddr === ANC) {
        ...
        collateralInfo = {
          ...
          collateralRatio:
            (valueConversion(collateralValue) /
              (valueConversion(asset.amount) * oraclePrice)) *
            100,
        };
      ...
      collateralInfo = {
        ...
        collateralRatio:
          (valueConversion(collateral.amount) /
            (valueConversion(asset.amount) * oraclePrice)) *
          100,
      };
    }
...
const getLockedInfo = async (idx) => {
  try {
    const lockInfo: any = await axios.get(MANTLE_URL + `?lockPositionInfo`, {
      params: {
...
export const getShortInfo = async (address: string) => {
  try {
    const mintInfo: any = await getMintInfo(address);
    if (mintInfo) {
      const positions = JSON.parse(
        mintInfo?.data?.data?.WasmContractsContractAddressStore?.Result
      )?.positions;
      const assetStatsPromise = getAssetsStats();
      const stakingRewardsPromise = getStakingRewards(address);

      const [assetStats, stakingRewards] = await Promise.all([
        assetStatsPromise,
      ...
      if (positions) {
        const totalShortInfo = await Promise.all(
          positions.map(async (position) => {
            ...
            const locked_amount =
              lockedInfo?.unlock_time > currentDate
                ? lockedInfo?.locked_amount
                : 0;
            const unlocked_amount =
              lockedInfo?.unlock_time < currentDate
                ? lockedInfo?.locked_amount
                : 0;
            const timeString =
              lockedInfo?.unlock_time > currentDate
                ? secondsToDate(lockedInfo?.unlock_time)
                : null;

            const collateralInfo = await getCollateralInfo(
              collateral,
              asset,
              oraclePrice
            );
            ...
            const shortInfo = {
              ...
              collateralInfo: {
                collateral: valueConversion(
                  collateralInfo?.collateral
                ).toString(),
                collateralValue: valueConversion(
                  collateralInfo?.collateralValue
                ).toString(),
                collateralRatio: (
                  collateralInfo?.collateralRatio ?? 0
                ).toString(),
