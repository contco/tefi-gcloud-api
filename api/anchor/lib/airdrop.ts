...
import {math, LCD_URL} from "@contco/terra-utilities";
import {networks} from "../../commons";
...
export const isAirdropsClaimed = async (address: string, stage: string) => {
  try {
   const {data}: any =  await axios.get(LCD_URL + `wasm/contracts/terra146ahqn6d3qgdvmj8cj96hh03dzmeedhsf0kxqm/store`, {
       params: {
...
export const getAirdrops = async (address: string) => {
  try {
    ...
    const result: any = await axios.get(anchorURI);
