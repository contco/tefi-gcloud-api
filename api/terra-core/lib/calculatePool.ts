...
import { getPrice, isLunaPair, isNoneNativePair, getLpValue, math, MICRO, FCD_URL } from "@contco/terra-utilities";
import { getTokenInfo, getPriceBySymbol } from "../../commons";
...
export const getPoolSymbol = async (poolresponse, isLuna = false) => {
    ...
    if (tokenContract && !isLuna) {
        coinInfo = await getTokenInfo(tokenContract)
...
export const calculatePoolData = async (poolResponses, userPoolBalances) => {
    const poolsData = [];
    let total = 0;
    const pricesRequest: any = await axios.get(FCD_URL + "v1/dashboard");
