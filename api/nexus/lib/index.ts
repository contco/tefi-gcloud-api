import { getPoolInfo, getPrice, getLatestBlockTime} from "@contco/terra-utilities";
...
const fetchData = (address: string) => {
    const result = Promise.all([fetchHoldings(address), getPoolInfo(NEXUS_CONTRACTS.pool), fetchAvailableLp(address), fetchStakedLp(address)]);
    return result;
}

export const getNexusAccount = async (address: string) => {
  const [holdingsInfo, poolInfo, availableLp, stakedLp] = await fetchData(address);
