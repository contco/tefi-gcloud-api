...
export const getPylonAccount = async (address: string) => {
    try {
        const [mineOverview, getAccountDetails, mineStakingData, minePoolData, governanceOverview, liquidityOverview, airdropData, pylonGateway] : any = await fetchPylonData(address);
        if (mineOverview) {
