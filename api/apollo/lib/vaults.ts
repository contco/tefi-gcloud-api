...
export const getVaultDetails = async () => {
  ...
  try {
    ...
    while (fetchMore) {
      ...
      const result = await wasmStoreRequest(
        apolloDaoContracts.vaultContract,
        query_msg
      );
...
export const getUserVaults = async (address: string) => {
  ...
  const user_vaults = strategies.filter(
    (item) => item.base_token_balance !== "0"
  );
  return user_vaults;
};
...
export const fetchVaultsData = async (address: string) => {
  try {
    const [vaultDetails, userVaults] = await Promise.all([
      getVaultDetails(),
