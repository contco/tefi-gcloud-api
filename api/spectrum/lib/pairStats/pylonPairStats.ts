...
const getPylonApy = async () => {
  try {
    const [govData, lpData]: any = await Promise.all([
...
export const calculatePylonPairStats = (
  pylonStatsData,
  poolInfos,
  govVaults,
  govConfig,
  ...
  const govWeight =
    govVaults.vaults.find((vault) => vault.address === contracts.pylonFarm)
      ?.weight || 0;
  const pairs = {};
