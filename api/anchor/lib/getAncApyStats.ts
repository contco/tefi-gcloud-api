...
export const getAnchorApyStats = async () => {
    try {
      const {data: distributionAPY}: any = await axios.get(ANCHOR_API_URL + "distribution-apy");
      const {data: ustLpRewardApy}: any = await axios.get(ANCHOR_API_URL + "ust-lp-reward");
      const {data: govRewardApy}: any= await axios.get(ANCHOR_API_URL + "gov-reward");
