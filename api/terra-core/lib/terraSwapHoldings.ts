...
export const fetchTerraSwapHoldings = async (
  address: string,
  lunaUstPrice?: string
) => {
  try {
    const bEthRequest = await fetchData(BASSETS_INFO + "beth");
    const [bEthInfo]: any = await Promise.all([bEthRequest]);
    ...
    const swapTasks = SWAP_TOKENS.map(async (item) => {
      ...
      const [balance, poolInfo] = await Promise.all([
        balancePromise,
        poolPromise,
      ]);
      ...
      if (balance !== "0") {
        ...
        let price = "";
        if (item.symbol !== "bETH") {
          price = item.isLunaPair
            ? convertPrice(poolPrice, lunaUstPrice)
            : poolPrice;
        } else {
