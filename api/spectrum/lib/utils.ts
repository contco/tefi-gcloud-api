...
import { wasmStoreRequest, LCD_URL } from "@contco/terra-utilities";
...
export const getPairPool = async (contract: string) => {
  const { data: pairPool }: any = await axios.get(
    LCD_URL + `wasm/contracts/${contract}/store`,
    {
      params: {
...
export const getRewardInfos = async (
  height: string,
