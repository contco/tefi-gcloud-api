...
export const calculatePoolDetails = (
  ...
  const poolValues = getPoolValues(
    stakedLpDetails,
    unstakedLpDetails,
    priceResult
  );
...
export const getMirrorAccount = async (address: string) => {
  try {
    ...
    const rewardsBalance =
      stakingPool && stakingRewards
        ? balance[BalanceKey.REWARD](stakingPool, stakingRewards)
        : null;
    const longStakedBalance = stakingRewards
      ? balance[BalanceKey.LPLONGSTAKED](stakingRewards)
      : null;
    const stakeableBalance = lpTokenBalance
      ? balance[BalanceKey.LPSTAKABLE](lpTokenBalance)
      : null;

    const mirPrice = pairsList ? price["pair"](pairsList)[MIR_TOKEN] : "0";
    const { mirrorAirdrops, airdropSum: mirrorAirdropSum } = formatAirdrops(
      airdropsData,
      mirPrice
    );
    ...
    const result = MIRROR_ASSETS.reduce((poolList, listing: ListedItem) => {
      const priceKey =
        listing.status === "LISTED" ? PriceKey.PAIR : PriceKey.END;
      const priceResult = pairsList
        ? price[priceKey](pairsList)[listing.token]
        : "0";
      ...
      if (stakingPool && pairsList && stakingRewards) {
        ...
        if (
          longStakedBalance?.[listing.token] ||
          stakeableBalance?.[listing.token]
        ) {
          ...
          const unstakedLpDetails = fromLP(
            stakeableAmount,
            shares,
            pairPool.total
          );
          if (
            stakedLpDetails?.asset?.amount !== "0" ||
            unstakedLpDetails?.asset?.amount !== "0"
          ) {
            ...
            mirrorPoolRewardsSum = plus(
              mirrorPoolRewardsSum,
              poolData.rewardsValue
            );
            poolList.push({
              symbol1: "UST",
