...
const getLoterraDraw = (
  ...
  if (userCombinations && userCombinations.length > 0) {
    ...
    const ticketPrice = math.div(
      loterraConfig?.price_per_ticket_to_register,
      MICRO
    );
...
export const getLoterraAccount = async (address: string) => {
  try {
    const [loterraConfig, lotaData] = await Promise.all([
      getLoterraConfig(),
      getLoterraStaking(address),
    ]);
    ...
    const loterraDraw = getLoterraDraw(
      userCombinations,
      loterraConfig,
      ticketCounts,
      jackpot
