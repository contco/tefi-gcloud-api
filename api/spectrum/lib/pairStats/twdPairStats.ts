...
export const getTwdPairStatsData = async (height: string, twdPoolResponse: any) => {
  try {
    const rewardInfoPromise = getRewardInfos(height, contracts.terraworldFarm, contracts.terraworldStaking);
    ...
    const [rewardInfo, farmConfig, twdLpApr, twdGovApr] = await Promise.all([rewardInfoPromise, farmConfigPromise, twdLpAprPromise, twdGovApyPromise]);
    return {rewardInfo, farmConfig, ...twdLpApr, ...twdGovApr};
  }
  catch(err) {
    return {rewardInfo: {bond_amount: '0'}, farmConfig: {community_fee: "0"},  apr: '0', apy: '0' }
  }
}

const getTwdLpApr = async(height: string, twdPoolResponse: any) => {
    try {
    const configTask = wasmStoreRequest(contracts.terraworldStaking, {config: {}});
    const stateTask = wasmStoreRequest(contracts.terraworldStaking, {state: {block_height: +height}});
    const [config, state] = await Promise.all([configTask, stateTask]);
    const twdPoolUSTAmount = twdPoolResponse.assets[1]?.info?.native_token?.['denom'] === UUSD_DENOM ? twdPoolResponse.assets[1].amount : twdPoolResponse.assets[0].amount;
...
const getTwdGovApy = async (height: string) => {
    try {
    const configTask = wasmStoreRequest(contracts.terraworldGov, {config: {}});
    const stateTask = wasmStoreRequest(contracts.terraworldGov, {state: {block_height: +height}}); 
    const [config, state] = await Promise.all([configTask, stateTask]);
    const current_distribution_schedule = (config.distribution_schedule as []).find(obj => +height >= +obj[0] && +height <= +obj[1]);
    const totalMint = +current_distribution_schedule[2];
  ...
  const createTwdPairStats = (poolApr: number, token: string, poolInfos: any, apy: string, govWeight: any, totalWeight: any ) => {
...
export const calculateTwdPairStats = (twdStatsData, poolInfos, govVaults, govConfig, terraSwapPoolResponses ) => {
    ...
    const govWeight = govVaults.vaults.find(vault => vault.address === contracts.terraworldFarm)?.weight || 0;
    const pairs = {};
    pairs[contracts.terraworldToken] = createTwdPairStats(poolApr, contracts.terraworldToken, poolInfos, apy, govWeight, totalWeight);
