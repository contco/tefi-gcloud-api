...
import gov from "../../anchor/lib/gov";
...
export const fetchUserGovStaking = async (address: string) => {
  try {
    ...
    const stakingResultPromise = contracts.govContracts.map(
      async (govContract: any) => {
        const stakingInfo = await wasmStoreRequest(
    ...
    const filteredResults = stakingResults.filter(
      (item) => item.bond_amount !== "0"
    );
...
export const getStarTerraGov = (userGovData: any, sttPrice: string) => {
  if (userGovData.length === 0) {
    ...
    return {
      govData,
      govRewardsTotal: govRewardsTotal.toString(),
      govStakedTotal: govStakedTotal.toString(),
    };
  }
};
