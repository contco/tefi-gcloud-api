...
import { mantleFetch, getLastSyncedHeight } from "./utils";
...
export const getTotalDesposit = async ({ address }: any) => {
  const totalDesposit = await anchor.earn.getTotalDeposit({
...
export default async (address) => {
  try {
    ...
    const apy =
      parseFloat(earnEpoch?.overseerEpochState?.deposit_rate) * blocksPerYear;
